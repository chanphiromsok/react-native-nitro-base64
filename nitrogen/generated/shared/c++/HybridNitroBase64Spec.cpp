///
/// HybridNitroBase64Spec.cpp
/// This file was generated by nitrogen. DO NOT MODIFY THIS FILE.
/// https://github.com/mrousavy/nitro
/// Copyright © Marc Rousavy @ Margelo
///

#include "HybridNitroBase64Spec.hpp"

namespace margelo::nitro::nitrobase64 {

  void HybridNitroBase64Spec::loadHybridMethods() {
    // load base methods/properties
    HybridObject::loadHybridMethods();
    // load custom methods/properties
    registerHybrids(this, [](Prototype& prototype) {
      prototype.registerHybridMethod("install", &HybridNitroBase64Spec::install);
      prototype.registerHybridMethod("encode", &HybridNitroBase64Spec::encode);
      prototype.registerHybridMethod("decode", &HybridNitroBase64Spec::decode);
      prototype.registerHybridMethod("decodeBuffer", &HybridNitroBase64Spec::decodeBuffer);
      prototype.registerHybridMethod("encodeBuffer", &HybridNitroBase64Spec::encodeBuffer);
    });
  }

} // namespace margelo::nitro::nitrobase64
